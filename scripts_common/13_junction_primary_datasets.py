...
def analyze_bam(path, read_len):
    ...
    for r in bam.fetch(CHROM, I4_START - 200, I4_END + 200):
        ...
        if any(s < B5 < e for s, e in blocks): ret5 += 1
        if any(s < B3 < e for s, e in blocks): ret3 += 1
    def meancov(a, b):
    ...
    bam.close()
    # retention count = mean of the reads crossing the two exon-intron junctions
    # (5' = exon4/intron, 3' = intron/exon5); the standard PIR "inclusion" measure.
    # ret5/ret3 are integers, so their mean can be a half-integer (e.g. 19,12->15.5).
    ret = (ret5 + ret3) / 2.0
    ...
    return (spliced, ret5, ret3, round(i4_cov, 1), round(ret, 1), round(pir_boundary, 1),
            round(ri203_cov, 1), round(ex4_cov, 1), round(cov_ratio, 1))

HEADER = ["sample", "dataset", "read_len_bp",
          "E4E5_junction_spanning_reads",
          "retention_reads_5prime_EIJ", "retention_reads_3prime_EIJ",
          "intron4_retention_reads_mean",
          "PIR_mean_5p3p_pct", "PIR_proximal_5prime_pct",
...
for label, bamname, sdir, rlen, ds in JOBS:
    ...
    if not os.path.exists(bam):
        ...
        continue
    spliced, ret5, ret3, i4cov, ret, pir_b, ri203cov, ex4cov, covratio = analyze_bam(bam, rlen)
    salmon203 = salmon_203_fraction(sdir, ds)
    # PIR is only meaningful when junction-spanning reads exist (they vanish at
    # 25 bp). Two variants: the 5'/3' MEAN (vast-tools-like, but note the two
    # boundaries are NOT equivalent here because retention is partial — 203/204
    # end inside the intron and never reach the 3' boundary), and the PROXIMAL
    # (5') PIR, the cleaner single number for this partial-retention locus.
    pir_mean = pir_b if spliced > 0 else "NA(no junction reads at this read len)"
    pir_prox = round(100 * ret5 / (ret5 + spliced), 1) if (spliced > 0) else "NA"
    rows.append([label, ds, rlen, spliced, ret5, ret3, ret, pir_mean, pir_prox,
...
for ds in sorted(set(r[1] for r in rows)):
    ...
    print(f"wrote {out}")
print()
hdr = (f"  {'sample':20s} {'ds':10s} {'rl':>3} {'E4E5j':>6} {'ret5':>5} {'ret3':>5} "
       f"{'PIRmean':>7} {'PIRprox':>7} {'covRatio%':>9} {'Salmon203%':>10}")
print(hdr)
for r in rows:
    pm = r[7] if isinstance(r[7], (int, float)) else "NA"
    pp = r[8] if isinstance(r[8], (int, float)) else "NA"
    print(f"  {r[0]:20s} {r[1]:10s} {r[2]:>3} {r[3]:>6} {r[4]:>5} {r[5]:>5} "
          f"{str(pm):>7} {str(pp):>7} {r[11]:>9} {r[12]:>10}")
